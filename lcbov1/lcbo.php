...
// Crawing Stores
$fp = fopen('/var/www/html/liakada/lcbo/store.json', 'w');
fwrite($fp, '{"results":[');
$link2 = file_get_contents('http://lcboapi.com/stores?page=1&access_key=MDphZDlkY2Y2NC01MTk4LTExZTUtODRiOC0wYmI3ZDJlYjRlNjE6bTlrdE44eHZpdFlkcEhQNEhHbGhUZERTRE1iUXJTSjRhbzBB');
$info2 = json_decode($link2, true);
$pages2 = $info2["pager"]["total_pages"];
$total2 = $info2["pager"]["total_record_count"];
$store = array();
$save2 = 0;
for ($j=1;$j<=$pages2;$j++) {
	$link2= file_get_contents('http://lcboapi.com/stores?page='.$j.'&access_key=MDphZDlkY2Y2NC01MTk4LTExZTUtODRiOC0wYmI3ZDJlYjRlNjE6bTlrdE44eHZpdFlkcEhQNEhHbGhUZERTRE1iUXJTSjRhbzBB');
	$stores = json_decode($link2, true);
	for ($i=0; $i<sizeof($stores["result"]); $i++) {
		$store[$i] = array(
			"id" => $stores["result"][$i]["id"],
			"is_dead" => $stores["result"][$i]["is_dead"],
			"name" => $stores["result"][$i]["name"],
			"address_line_1" => $stores["result"][$i]["address_line_1"],
			"address_line_2" => $stores["result"][$i]["address_line_2"],
			"city" => $stores["result"][$i]["city"],
			"postal_code" => $stores["result"][$i]["postal_code"],
			"telephone" => $stores["result"][$i]["telephone"],
			"fax" => $stores["result"][$i]["fax"],
			"latitude" => $stores["result"][$i]["latitude"],
			"longitude" => $stores["result"][$i]["longitude"],
			"tags" => $stores["result"][$i]["tags"],
			"has_parking" => $stores["result"][$i]["has_parking"],
			"has_special_occasion_permits" => $stores["result"][$i]["has_special_occasion_permits"],
			"sunday_open" => $stores["result"][$i]["sunday_open"],
			"sunday_close" => $stores["result"][$i]["sunday_close"],
			"monday_open" => $stores["result"][$i]["monday_open"],
			"monday_close" => $stores["result"][$i]["monday_close"],
			"tuesday_open" => $stores["result"][$i]["tuesday_open"],
			"tuesday_close" => $stores["result"][$i]["tuesday_close"],
			"wednesday_open" => $stores["result"][$i]["wednesday_open"],
			"wednesday_close" => $stores["result"][$i]["wednesday_close"],
			"thursday_open" => $stores["result"][$i]["thursday_open"],
			"thursday_close" => $stores["result"][$i]["thursday_close"],
			"friday_open" => $stores["result"][$i]["friday_open"],
			"friday_close" => $stores["result"][$i]["friday_close"],
			"saturday_open" => $stores["result"][$i]["saturday_open"],
			"saturday_close" => $stores["result"][$i]["saturday_close"],
			"updated_at" => $stores["result"][$i]["updated_at"],
			"created_at" => $stores["result"][$i]["created_at"]
		);
		$save2 = $save2 + 1;
